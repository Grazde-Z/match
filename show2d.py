...
def main():
    ...
    #             if k==777 and int(MappointFrameAsso[i][1])==1218:
    #                 print(pixel_coords)
    ...
    for i in range(len(poses)):
        ...
        for k in range(len(MaplineFrameAsso)):
            if MaplineFrameAsso[k][1]==i:
                ...
                # num_v = random.randint(-1, 1)
                # v_2=int(fr[5])+num_v
                u_1=round(MaplineFrameAsso[k][2])
                v_1=round(MaplineFrameAsso[k][3])
                u_2=round(MaplineFrameAsso[k][5])
                v_2=round(MaplineFrameAsso[k][6])
                num=round(MaplineFrameAsso[k][0])
                cv2.line(white_image,(u_1,v_1),(u_2,v_2),(0,100,0),2,cv2.LINE_AA)            
                cv2.putText(white_image, str(num), (u_1,v_1), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(100,100,0), 1, cv2.LINE_AA)
        cv2.imwrite(f"/home/arondight/G.Z/Open-structure/show2d.png",white_image)
        print(i)
